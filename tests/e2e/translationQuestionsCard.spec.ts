...
test.describe("translationQuestions Card", () => {
  ...
  test("Making font size smaller", async ({ page }) => {
    await homePage.cardMenu("#resource_card_tq_card_menu");
  ...
  test("Buttons are visible", async ({ page }) => {
    await expect(
      page.locator("#minimize_button_resource_card_tq")
    ).toBeVisible();
    await expect(
      page.locator("#markdown_button_resource_card_tq")
    ).toBeVisible();
    await expect(page.locator("#save_button_resource_card_tq")).toBeVisible();
    await expect(
      page.locator("#resource_card_tq").getByLabel("Update my content")
    ).toBeVisible();
    await expect(
      page
        .locator("#resource_card_tq")
    ...
    await expect(page.locator("#resource_card_tq_card_menu")).toBeVisible();
  });
});
